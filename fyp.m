...
% Last Modified by GUIDE v2.5 17-May-2015 05:25:41
...
fid = fopen('data.csv');                       
cts = textscan(fid, '%s %f %f %f %f', 'Delimiter', ',');  
d = datenum(cts{1});                             
v = [cts{2} cts{3} cts{4} cts{5}] ;                                      
fclose(fid); 
plot(d, v), datetick('x')
grid on
legend('Solar Power','Grid Voltage' ,'Battery Level','Load Consumption')
